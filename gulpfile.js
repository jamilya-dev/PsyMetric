...
/**
 * CSS - добавляем задачи для обработки CSS файлов
 */
// Для добавления:
gulp.task('css:clean', lazyQuire(require, './gulp/core/recipes/css/clean')); // Добавьте файл для очистки, если нужен
gulp.task('css:dev', gulp.series('css:clean', lazyQuire(require, './gulp/core/recipes/css/dev')));
gulp.task('css:prod', gulp.series('css:clean', lazyQuire(require, './gulp/core/recipes/css/prod')));
gulp.task('css:watch', gulp.series('css:dev', lazyQuire(require, './gulp/core/recipes/css/watch')));
...
gulp.task('build', gulp.parallel('fonts:prod', 'svg:prod', 'sprite:prod', 'images:prod', 'scripts:prod', 'styles:prod'));
